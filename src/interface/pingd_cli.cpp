...
void cmd_download(const std::string &);
void cmd_upload(const std::string &);
...
static const std::map<std::string, command_t> commands = {
    ...
    { "download", { cmd_download, "download [file on server] [location on client]: sends file from server to client" } },
    { "upload", { cmd_upload, "upload [location on server] [file on client]: receives a file from the client and saves it on the server." } },
...
void cmd_list(const std::string &) {
    ...
    while (std::getline(connections, connection)) {
        ...
        auto res = split_input(connection, "/");
        ...
        std::cout << "\tConnection: " << tmp << "\n";
        active_connections.insert(tmp);
    }
...
void cmd_download(const std::string &input) {
    auto input_arr = split_input(input, " ");

    if (input_arr.size() < 3) {
        std::puts("usage: download [file on server] [location on client]");
        return;
    }
...
void cmd_upload(const std::string &input) {
    auto input_arr = split_input(input, " ");

    if (input_arr.size() < 3) {
        std::puts("usage: upload [location on server] [file on client]");
        return;
    }
    ...
    if (!cur_host.empty()) {
        std::cout << "Receiving file: " << input_arr.at(2) << " to: " << input_arr.at(1) << " on: "
                  << cur_host
                  << "\n";
